...
<li>可自定义任意数量的IPv4目标网址/网段数据集合，以在mwan3的流量出口策略规则中使用。</li>
<li>可自定义任意数量的域名数据集合，按所访问的域名分配流量出口。</li>
...
1.下载本工具的软件压缩包“lzrules-[version ID].tgz”（例如：lzrules-v1.0.8.tgz）。

2.使用WinSCP等工具将压缩包上传至路由器的任意目录。

3.在SSH终端中使用解压缩命令在当前目录中将软件解压缩，生成lzrules-[version ID]目录（例如：lzrules-v1.0.8），进入其中可看到一个lzrules目录，此为脚本的工作目录。
```markdown
